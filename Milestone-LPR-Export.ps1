...
# 📊 Export Excel con immagini
$excelFile = "$base\${serverName}_Eventi_LPR_$timestampFolder.xlsx"
$excel = $report | Export-Excel -Path $excelFile -WorksheetName "LPR" -AutoSize -PassThru
$ws = $excel.Workbook.Worksheets["LPR"]

$row = 2
foreach ($r in $report) {
    if ($r.Immagine -and (Test-Path $r.Immagine)) {
        $img = [System.Drawing.Image]::FromFile($r.Immagine)
...
Write-Host "`n✅ Esportazione completata! File: $excelFile" -ForegroundColor Green
